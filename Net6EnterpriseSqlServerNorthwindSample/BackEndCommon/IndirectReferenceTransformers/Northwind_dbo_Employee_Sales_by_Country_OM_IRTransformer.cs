/* **** This file and its contents were generated by YouGen Sharp on (UTC) 20230307195906 on behalf of: ****
**** You Gen Software LLC ****
**** 5961 BeaconPark St ****
**** Lithia Florida 33547 United States of America ****
**** whose account is uniquely identified by: 716881407 ****
**** This file and its contents are subject to the conditions of use for the Enterprise Tier License as specified at: https://www.yougensoft.com/en/conditions-of-use. ****
**** This comment block must not be removed. ****
 */
using Northwind_BackEndSqlEntities.Entities;
using Northwind_BackEndCommon.Services;
using Northwind_Common.IndirectReferenceTransformerModels;
namespace Northwind_BackEndCommon.IndirectReferenceTransformers;
public partial class IRTransformers : IIRTransformers
{
	public Northwind_dbo_Employee_Sales_by_Country_OM_IR ToIndirectModel(Northwind_dbo_Employee_Sales_by_Country_OM input)
	{
		var retData = new Northwind_dbo_Employee_Sales_by_Country_OM_IR(
			country_ : input.Country,
			lastName_ : input.LastName,
			firstName_ : input.FirstName,
			shippedDate_ : input.ShippedDate,
			orderID_IR_ : _encryptionDecryptionService!.EncInt32(input.OrderID),
			saleAmount_ : input.SaleAmount
		);
		return retData;
	}
	public Northwind_dbo_Employee_Sales_by_Country_OM ToEntity(Northwind_dbo_Employee_Sales_by_Country_OM_IR input)
	{
		var retData = new Northwind_dbo_Employee_Sales_by_Country_OM(		
			country_ : input.Country,
			lastName_ : input.LastName ?? String.Empty,
			firstName_ : input.FirstName ?? String.Empty,
			shippedDate_ : input.ShippedDate,
			orderID_ : _encryptionDecryptionService.DecInt32(input.OrderID_IR),
			saleAmount_ : input.SaleAmount
		);
		return retData;
	}
}
