...
public class Northwind_dbo_Sales_by_Category_RequestHandler : INorthwind_dbo_Sales_by_Category_RequestHandler
{
    ...
    private readonly Northwind_dbo_Sales_by_Category_IR_FluentValidator _readValidator;
    public Northwind_dbo_Sales_by_Category_RequestHandler(
        ...
        ,Northwind_dbo_Sales_by_Category_IR_FluentValidator readValidator
    )
    {
        _logger = logger;
	...
	//Main Handlers
	public async Task<IEnumerable<Northwind_dbo_Sales_by_Category_IR>?> HandleGetAll()
	{
		await PreHandleGetAll();
		var retData = await _repository.GetAll();
		await PostHandleGetAll();
		return retData == null || !retData.Any() ? Enumerable.Empty<Northwind_dbo_Sales_by_Category_IR>() : retData.Select(x => _indirectReferenceTransformers.ToIndirectModel(x)!).ToArray().ToList();
	}
	//PreCRUD Handlers
