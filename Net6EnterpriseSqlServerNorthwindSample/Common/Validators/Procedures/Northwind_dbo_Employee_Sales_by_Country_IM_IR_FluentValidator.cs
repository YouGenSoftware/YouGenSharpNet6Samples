...
public class Northwind_dbo_Employee_Sales_by_Country_IM_IR_FluentValidator : AbstractValidator<Northwind_dbo_Employee_Sales_by_Country_IM_IR>, IFluentValidator_Base
{
    public Northwind_dbo_Employee_Sales_by_Country_IM_IR_FluentValidator()
    {
			RuleFor(x => x.Beginning_Date)
				.NotEmpty();
			RuleFor(x => x.Ending_Date)
				.NotEmpty();
    }
    public async Task<IEnumerable<String>> ValidateValue(Object model, String propertyName)
    {
        var result = await ValidateAsync(ValidationContext<Northwind_dbo_Employee_Sales_by_Country_IM_IR>.CreateWithOptions((Northwind_dbo_Employee_Sales_by_Country_IM_IR)model, x => x.IncludeProperties(propertyName)));
        if (result.IsValid)
    ...
    public async Task ValidateModelAndThrow(Object model)
    {
        try
        {
            await this.ValidateAndThrowAsync((Northwind_dbo_Employee_Sales_by_Country_IM_IR)model);
        }
        catch (Exception ex)
        {
#if DEBUG
