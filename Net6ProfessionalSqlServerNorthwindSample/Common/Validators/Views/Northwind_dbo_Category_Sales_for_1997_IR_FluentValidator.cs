...
public class Northwind_dbo_Category_Sales_for_1997_IR_FluentValidator : AbstractValidator<Northwind_dbo_Category_Sales_for_1997_IR>, IFluentValidator_Base
{
    public Northwind_dbo_Category_Sales_for_1997_IR_FluentValidator()
    {
			// RuleFor(x => x.CategoryName)
				// .NotEmpty();
			// RuleFor(x => x.CategorySales)
				// .NotEmpty();
    }
    public async Task<IEnumerable<String>> ValidateValue(Object model, String propertyName)
    {
        var result = await ValidateAsync(ValidationContext<Northwind_dbo_Category_Sales_for_1997_IR>.CreateWithOptions((Northwind_dbo_Category_Sales_for_1997_IR)model, x => x.IncludeProperties(propertyName)));
        if (result.IsValid)
    ...
    public async Task ValidateModelAndThrow(Object model)
    {
        try
        {
            await this.ValidateAndThrowAsync((Northwind_dbo_Category_Sales_for_1997_IR)model);
        }
        catch (Exception ex)
        {
#if DEBUG
