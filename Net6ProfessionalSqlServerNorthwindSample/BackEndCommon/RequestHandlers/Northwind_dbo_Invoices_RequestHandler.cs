...
public class Northwind_dbo_Invoices_RequestHandler : INorthwind_dbo_Invoices_RequestHandler
{
    private readonly ILogger<Northwind_dbo_Invoices_RequestHandler> _logger;
    ...
    private readonly INorthwind_dbo_Invoices_Repository _repository;
    private readonly Northwind_dbo_Invoices_IR_FluentValidator _readValidator;
    public Northwind_dbo_Invoices_RequestHandler(
        ILogger<Northwind_dbo_Invoices_RequestHandler> logger
        ,IEncryptionDecryptionService encryptionDecryptionService
        ,IIRTransformers indirectReferenceTransformers
        ,INorthwind_dbo_Invoices_Repository repository
        ,Northwind_dbo_Invoices_IR_FluentValidator readValidator
    )
    {
        _logger = logger;
	...
	//Main Handlers
	public async Task<IEnumerable<Northwind_dbo_Invoices_IR>?> HandleGetAll()
	{
		await PreHandleGetAll();
		var retData = await _repository.GetAll();
		await PostHandleGetAll();
		return retData == null || !retData.Any() ? Enumerable.Empty<Northwind_dbo_Invoices_IR>() : retData.Select(x => _indirectReferenceTransformers.ToIndirectModel(x)!).ToArray().ToList();
	}
	//PreCRUD Handlers
