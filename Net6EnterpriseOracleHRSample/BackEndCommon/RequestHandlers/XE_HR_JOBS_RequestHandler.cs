...
public class XE_HR_JOBS_RequestHandler : IXE_HR_JOBS_RequestHandler
{
    ...
    private readonly XE_HR_JOBS_IR_FluentValidator_Create _createValidator;
    private readonly XE_HR_JOBS_IR_FluentValidator_Read _readValidator;
    private readonly XE_HR_JOBS_IR_FluentValidator_Update _updateValidator;
    private readonly XE_HR_JOBS_IR_FluentValidator_Delete _deleteValidator;
    public XE_HR_JOBS_RequestHandler(
        ...
        ,XE_HR_JOBS_IR_FluentValidator_Create createValidator
        ,XE_HR_JOBS_IR_FluentValidator_Read readValidator
        ,XE_HR_JOBS_IR_FluentValidator_Update updateValidator
        ,XE_HR_JOBS_IR_FluentValidator_Delete deleteValidator
    )
    {
        _logger = logger;
	...
	//Main Handlers
	public async Task<IEnumerable<XE_HR_JOBS_IR>?> HandleGetAll()
	{
		await PreHandleGetAll();
		var retData = await _repository.GetAll();
		await PostHandleGetAll();
		return retData == null || !retData.Any() ? Enumerable.Empty<XE_HR_JOBS_IR>() : retData.Select(x => _indirectReferenceTransformers.ToIndirectModel(x)!).ToList();
	}
	public async Task<IEnumerable<XE_HR_JOBS_IR>?> HandleGetByJOB_ID(String jOB_ID)
	{
	...
	public async Task<XE_HR_JOBS_IR?> HandleCreate<T>(T irModel) where T : XE_HR_JOBS_IR
	{
	...
	public async Task HandleUpdateByJOB_ID<T>(String jOB_ID, T irModel) where T : XE_HR_JOBS_IR
	{
		var entity = _indirectReferenceTransformers.ToEntity(irModel);
	...
	{
		// Add preCRUD calls/events
		var selectorProperties = new XE_HR_JOBS_IR();
		selectorProperties.JOB_ID = jOB_ID;
		await _readValidator.ValidateModelAndThrow(selectorProperties);
	}
	private async Task PreHandleCreate(XE_HR_JOBS_IR irModel)
	{
		// Add preCRUD calls/events
		await _createValidator.ValidateModelAndThrow(irModel);
	}
	private async Task PreHandleUpdateByJOB_ID(String jOB_ID, XE_HR_JOBS_IR irModel)
	{
		// Add preCRUD calls/events
		var selectorProperties = new XE_HR_JOBS_IR();
		selectorProperties.JOB_ID = jOB_ID;
	...
	private async Task PostHandleCreate(XE_HR_JOBS_IR irModel)
	{
		// Add postCRUD calls/events
		// Remove the line below after adding optional async logic
		await Task.CompletedTask;
	}
	private async Task PostHandleUpdateByJOB_ID(String jOB_ID, XE_HR_JOBS_IR irModel)
	{
		// Add postCRUD calls/events
		// Remove the line below after adding optional async logic
		await Task.CompletedTask;
