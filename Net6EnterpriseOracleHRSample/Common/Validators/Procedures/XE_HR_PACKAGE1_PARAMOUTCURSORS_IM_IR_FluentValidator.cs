...
public class XE_HR_PACKAGE1_PARAMOUTCURSORS_IM_IR_FluentValidator : AbstractValidator<XE_HR_PACKAGE1_PARAMOUTCURSORS_IM_IR>, IFluentValidator_Base
{
    public XE_HR_PACKAGE1_PARAMOUTCURSORS_IM_IR_FluentValidator()
    {
			RuleFor(x => x.VAR_INPUT)
				.NotEmpty();
			RuleFor(x => x.VAR_DATE_INPUT)
				.NotEmpty();
			RuleFor(x => x.VAR_INPUT)
				.NotEmpty();
			RuleFor(x => x.VAR_DATE_INPUT)
    ...
    {
        var result = await ValidateAsync(ValidationContext<XE_HR_PACKAGE1_PARAMOUTCURSORS_IM_IR>.CreateWithOptions((XE_HR_PACKAGE1_PARAMOUTCURSORS_IM_IR)model, x => x.IncludeProperties(propertyName)));
        if (result.IsValid)
    ...
    public async Task ValidateModelAndThrow(Object model)
    {
        try
        {
            await this.ValidateAndThrowAsync((XE_HR_PACKAGE1_PARAMOUTCURSORS_IM_IR)model);
        }
        catch (Exception ex)
        {
#if DEBUG
